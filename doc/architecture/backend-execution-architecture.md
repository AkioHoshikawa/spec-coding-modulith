...
- 長時間実行される処理（外部API呼び出し、バッチ処理）
- 処理の疎結合化が必要な場合（出荷通知、メール送信）

### 3.3 外部システム連携パターン

#### REST API クライアント
- **フレームワーク**: Spring WebFlux WebClient
- **用途**: 
  - 決済プロバイダAPI（トークン決済、与信処理）
  - 3PL/配送業者API（出荷指示、配送追跡）
  - 外部ERP/会計システム（注文データエクスポート）
- **レジリエンスパターン**: 
  - タイムアウト設定（接続: 3秒、読み取り: 10秒）
  - リトライ（指数バックオフ、最大3回）
  - サーキットブレーカー（将来的に Resilience4j 導入検討）
...
3. **Saga パターン** (将来的な実装候補):
   - 複雑な分散トランザクション（返品・交換フロー等）
   - 補償トランザクションによるロールバック
...
現在未実装だが、以下のキャッシング戦略を検討：
- **商品マスタ**: Redis（TTL: 1時間、Write-Through）
- **在庫数**: Redis（TTL: 30秒、Read-Through + Invalidation）
- **セッション情報**: Redis（Spring Session）
...
- **閾値**: 5秒間で50%以上のエラー率でOpen
- **リセット時間**: 30秒後にHalf-Openへ遷移
...
- **デッドレターキュー**: 3回失敗したイベントは別テーブルに移動（将来実装）

#### 在庫整合性制御
- **楽観ロック**: 通常の在庫更新（バージョン番号チェック）
- **悲観ロック**: ピーク時の在庫引当（FOR UPDATE）
- **負数防止**: CHECK制約またはアプリケーションレベルでの検証（BR-008）
...
- **JWT (JSON Web Token)**: ステートレス認証
  - アクセストークン: 15分（短寿命）
  - リフレッシュトークン: 7日間
- **OAuth 2.0 / OpenID Connect**: 外部プロバイダ連携（Google, Apple等）
...
- **ロールベースアクセス制御 (RBAC)**: 
  ...
  - 管理者（ADMIN）
  - システム（SYSTEM）
- **API保護**: Spring Security + JWT検証フィルター
...
- **通信**: TLS 1.2+ 必須（HTTPS）
- **保存データ**: 
  - パスワード: Argon2 / PBKDF2（ソルト付きハッシュ）
  - 決済情報: トークン化（PCI-DSS準拠）
  - 個人情報: 必要に応じてカラムレベル暗号化

#### 監査ログ
- **操作ログ**: 注文操作、在庫変更、管理操作を記録
- **保持期間**: 最低1年間
- **ログ項目**: ユーザーID、操作種別、対象エンティティ、タイムスタンプ、IPアドレス
...
#### ログレベル
- **DEBUG**: Spring Modulith内部動作（`org.springframework.modulith: DEBUG`）
- **INFO**: ビジネスイベント（注文確定、在庫引当等）
- **WARN**: 一時的エラー、リトライ発生
- **ERROR**: ビジネスロジックエラー、システムエラー

#### 構造化ログ（将来実装）
- **フォーマット**: JSON形式
- **フィールド**: timestamp, level, logger, message, traceId, spanId, userId, orderId

### 7.2 メトリクス（将来実装）

#### アプリケーションメトリクス
- **Spring Boot Actuator**: エンドポイント公開（`/actuator/health`, `/actuator/metrics`）
- **Micrometer**: メトリクス収集フレームワーク
- **主要メトリクス**:
  - API応答時間（P50, P95, P99）
  - イベント処理時間
  - 在庫引当成功率
...
- **Spring Cloud Sleuth + Zipkin**: リクエストフロー追跡
- **TraceID**: リクエスト全体を通じた一意識別子
- **SpanID**: 各処理単位の識別子
- **用途**: パフォーマンス分析、エラー原因調査
...
#### アプリケーションサーバー
- **ランタイム**: Java 17 (LTS)
- **フレームワーク**: Spring Boot 3.5.6（組み込みTomcat / Netty）
- **起動方式**: Executable JAR（`java -jar app.jar`）
- **プロセス管理**: systemd / Docker / Kubernetes

#### リソース要件（推奨）
- **CPU**: 2 vCPU（最小）、4 vCPU（推奨）
- **メモリ**: 2GB（最小）、4GB（推奨）
  - JVM Heap: `-Xms1G -Xmx2G`
  - Metaspace: `-XX:MetaspaceSize=256M -XX:MaxMetaspaceSize=512M`
- **ディスク**: 10GB（ログ、一時ファイル含む）

### 8.2 スケーリング戦略

#### 水平スケーリング
- **ステートレス設計**: セッション情報はRedis等外部ストアに保存
- **ロードバランサー**: L7 LB（Nginx, AWS ALB等）
- **スケールアウト条件**: 
  - CPU使用率 > 70%
  - API P99レイテンシ > 2秒
  - 同時リクエスト数 > 1000/秒
...
- **開発環境 (DEV)**: ローカル開発、H2インメモリDB
- **ステージング環境 (STG)**: 本番相当のインフラ、PostgreSQL
- **本番環境 (PROD)**: 冗長化構成、マルチAZ配置
...
#### エンドポイント
- **Liveness Probe**: `/actuator/health/liveness`（プロセス生存確認）
- **Readiness Probe**: `/actuator/health/readiness`（トラフィック受付可否）
...
#### イベントデータバックアップ
- Event Publication Tableのバックアップも同時実施
- 未完了イベントのリストア後、自動再発行

### 9.3 障害復旧手順

1. **アプリケーション再起動**: 未完了イベントの自動リトライ
2. **データベースリストア**: 最新バックアップからリカバリ
3. **整合性チェック**: 在庫数、注文ステータスの突合
4. **補償処理**: 不整合データの手動修正または補償トランザクション実行

## 10. 将来的な拡張ポイント

### 10.1 マイクロサービス化への移行

現在のモジュール境界を維持したまま、必要に応じて以下のマイクロサービス抽出が可能：
- **Order Service**: 注文ドメイン（model.order）
- **Inventory Service**: 在庫ドメイン（model.inventory）
- **Communication**: RESTful APIまたはメッセージブローカー（Kafka, RabbitMQ）

### 10.2 イベント外部化

- **Message Broker**: Kafka, RabbitMQへのイベント転送
- **用途**: 
  - データウェアハウスへのリアルタイム連携
  - 外部システム（会計、CRM）への通知
  - イベントソーシング基盤への移行
...
- **コマンド側**: 書き込み最適化（正規化スキーマ）
- **クエリ側**: 読み取り最適化（非正規化、Read Model）
- **同期方式**: イベント駆動でRead Modelを更新
...
| モジュール管理 | Spring Modulith | 1.4.1 |
| Webフレームワーク | Spring WebFlux | (Spring Boot 3.5.6) |
| ORM | Spring Data JPA + Hibernate | (Spring Boot 3.5.6) |
| データベース | H2 (開発) / PostgreSQL (本番想定) | - |
| ビルドツール | Gradle | 8.x |
| テストフレームワーク | JUnit 5 + Spring Test | (Spring Boot 3.5.6) |
