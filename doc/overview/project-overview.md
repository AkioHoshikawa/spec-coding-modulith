...
- プロモーション（簡易クーポン）対応
- リアルタイム在庫引当と在庫ロック機構
- 注文ステータス管理（受注、引当、出荷、完了、キャンセル）
- 管理画面（注文一覧、在庫状況、クーポン管理）
 - ログイン／アカウント管理（ユーザー登録、認証）
...
## 6. ハイレベルデータモデル
- 主要エンティティ：User、Order、OrderLine、SKU、Inventory、Coupon
...
- 在庫引当：短時間の強整合を必要とするため、アプリケーション側でのロック設計（例：楽観ロック）を推奨
